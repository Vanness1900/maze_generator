...
class DQN(nn.Module):
    """Deep Q-Network - slightly bigger for complex maze."""
...
class MazeEnv:
    ...
    def _get_state(self):
        """Enhanced state with wall awareness."""
        r, c = self.pos
        gr, gc = self.goal
        ...
        # Count nearby walls (helps avoid getting stuck)
        walls_count = 0
        open_count = 0
...
class PygameVisualizer:
    ...
    @property
    def cell_size(self):
        """Current cell size with zoom applied."""
        return int(self.base_cell_size * self.zoom_level)
    
    def draw_maze(self):
        """Draw the maze with thin walls."""
        self.maze_surface.fill(self.BLACK)
        
        cell_size = self.cell_size
        wall_thickness = max(1, cell_size // 5)  # Wall is 1/5th of cell size
...
def train_dqn(env, config):
    """Train DQN agent with adaptive stopping."""
    ...
    # Post-training summary across 10 deciles
...
def plot_results(decile_stats):
    """Create matplotlib summary plots based on decile sampling."""
    if not decile_stats:
...
def evaluate_model(env, model, num_episodes=100):
    """Test the trained model with epsilon=0 (no exploration)."""
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.eval()
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            # ε=0: Always pick best action (no random exploration)
            with torch.no_grad():
                state_t = torch.FloatTensor(state).unsqueeze(0).to(device)
